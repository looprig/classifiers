...
# Adapted from github.com/looprig/tests scripts/check-release-modfile.sh.
#
# Rejects any `replace` directive in a release modfile whose target is a
# local filesystem path, file URL, or unversioned string, so a tagged
# release build can never silently pick up a developer's local sibling
# checkout (e.g. `replace github.com/looprig/harness => ../harness-...`).
# Accepts tagged-version replacements and plain requires.
